...
-- 1. Add unique_sources_count to places table
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'places' AND column_name = 'unique_sources_count') THEN
        ALTER TABLE public.places ADD COLUMN unique_sources_count INT DEFAULT 0;
    END IF;
END $$;

-- 2. Update ingest_signal RPC to enforce single-source cap
...
BEGIN
    ...
    -- E. Log history snapshot (Legacy/Backward Compatibility)
    BEGIN
